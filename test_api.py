...
def run_tests():
    ...
    with open('test_results.txt', 'w', encoding='utf-8') as file:
        file.write('')
        file.write(f"Проверенные запросы:\n")
        file.write(f"\n")
...
def send_email():
    ...
    # Фильтрация запросов только с "🔴"
    red_lines = [line for line in lines if "🔴" in line]

    # Формирование текста письма с первой строкой "Проверенные запросы:" и данными из запросов с "🔴"
    content = "Запросы с ошибками:\n" + ''.join(red_lines)

    # Если есть запросы с маркером "🔴", добавить отступ и строку "Детализация во вложении"
    if len(red_lines) > 0:
        content += "\n\n"
        content += "Детализация во вложении"

    # Если нет запросов с маркером "🔴", изменить текст письма
    if len(red_lines) == 0:
        content = "Все запросы скрипта отработали в штатном режиме, детальная информация во вложении."
